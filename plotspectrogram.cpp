...
public:
    ...
    SpectrogramData(QVector<QVector<double> > in_data) {
        array2d_data = in_data;
        array2d_row = in_data.size();
        if (array2d_row > 0)
            array2d_col = in_data[0].size();
        else
            array2d_col = 0;
...
//        qDebug()<<"max value = "<<max_val;
    ...
    virtual QwtRasterData* copy() const {
        ...
        clone->setArray2DRow(array2d_row);
        clone->setArray2DCol(array2d_col);
        clone->setArray2dData(array2d_data);
    ...
    void setArray2DRow(int row) { array2d_row = row; }
    void setArray2DCol(int col) { array2d_col = col; }
    void setArray2dData(QVector<QVector<double> > array) { array2d_data = array; }

private:
    QVector<QVector<double> > array2d_data;
    int array2d_row;
    int array2d_col;
};
...
PlotSpectrogram::PlotSpectrogram(QVector<QVector<double> > array, QWidget *parent):
    QwtPlot( parent ),
    d_alpha(255)
{
    ...
    for ( double level = 0.005; level < 10.0; level += 0.001 )
        contourLevels += level;
    d_spectrogram->setContourLevels( contourLevels );

    d_spectrogram->setData( new SpectrogramData(array) );
    ...
    setColorMap( PlotSpectrogram::RGBMap );
...
void PlotSpectrogram::showContour( bool on )
{
    d_spectrogram->setDisplayMode( QwtPlotSpectrogram::ContourMode, on );
    replot();
}

void PlotSpectrogram::showSpectrogram( bool on )
{
...
void PlotSpectrogram::setColorMap( int type )
{
...
//void PlotSpectrogram::printPlot()
//{
